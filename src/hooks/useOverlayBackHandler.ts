 ...
 * Uses delayed history cleanup to prevent popstate collision between consecutive overlays.
 */
export const useOverlayBackHandler = (isOpen: boolean, onClose: () => void) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (isPushedRef.current) {
        isPushedRef.current = false;
        // Delay history cleanup tick to avoid popstate collision during direct modal transitions
        setTimeout(() => {
