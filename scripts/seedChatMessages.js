...
if (!serviceAccountPath) {
  ...
  console.log('Usage: FIREBASE_SERVICE_ACCOUNT_PATH=key.json node scripts/seedChatMessages.js [channelId] [count]');
  console.log('------------------------------------------------------------');
} else {
...
async function seedChatMessages(dryRun = true) {
  ...
  for (let i = 1; i <= messageCount; i++) {
    ...
    const msgTimestamp = admin.firestore.Timestamp.fromMillis(baseTime + (i * 60 * 1000));
    ...
    const messageData = {
      ...
      senderRole: 'student',
      content: `[TEST SEED #${i}] This is synthetic historical message #${i} for pagination testing.`,
      reactionCounts: {},
      status: 'active',
    ...
    if (dryRun) {
      if (i <= 5 || i > messageCount - 3) {
        console.log(` -> [DRY RUN] Message #${i}: ID=${msgId}, Time=${msgTimestamp.toDate().toISOString()}`);
      } else if (i === 6) {
  ...
  console.log(`\nSeeding complete. ${messageCount} test messages created in #${targetChannelId}.`);
}
